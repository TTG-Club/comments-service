...
public class CommentService
{
    ...
    /**
     * Приводит снимок имени автора к переданному отображаемому имени. Вызывается при смене
     * имени пользователем (тогда обновляются старые комментарии) и после публикации нового
     * комментария (он стамперит логин из токена — этот вызов заменяет его на актуальное имя).
     * Идемпотентно: повторный вызов с тем же именем вернёт 0.
     * <p>
     * {@code sourcePlatform} ограничивает переименование одной платформой ({@code null} — все):
     * {@code authorId} общий у пользователя на всех сайтах, а имя у каждого сайта своё, поэтому
     * сайт правит только свои комментарии.
     *
     * @return сколько комментариев переименовано; 0, если менять было нечего
     */
    @Transactional
    public int renameAuthor(
            final UUID authorId,
            final SourcePlatform sourcePlatform,
            final String displayName
    )
    {
        return commentRepository.renameAuthor(authorId, sourcePlatform, displayName.trim());
    }
