...
public class CommentService
{
    ...
    /**
     * Приводит снимок имени автора во всех его комментариях к переданному отображаемому
     * имени. Вызывается при смене имени пользователем (тогда обновляются старые комментарии)
     * и после публикации нового комментария (он стамперит логин из токена — этот вызов
     * заменяет его на актуальное имя). Идемпотентно: повторный вызов с тем же именем вернёт 0.
     *
     * @return сколько комментариев переименовано; 0, если менять было нечего
     */
    @Transactional
    public int renameAuthor(final UUID authorId, final String displayName)
    {
        return commentRepository.renameAuthor(authorId, displayName.trim());
    }
